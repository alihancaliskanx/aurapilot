...
void Sub::do_nav_wp(const AP_Mission::Mission_Command& cmd)
{
    ...
    // NOT: "alt == 0 -> mevcut irtifayi kullan" (Copter mirasi) KALDIRILDI.
    // Sub'da 0 = SATIH, "irtifa verilmedi" degil: satha cikis waypoint'i (alt=0)
    // o anki derinlige snap'lenip sifir uzunluklu bacak oluyor, WP aninda
    // "ulasildi" sayiliyor ve arac satha cikmadan hold suresince bekliyordu
    // (Egitim/komutanlik loglari 00000030/31.BIN). master bb0d13ca55'in portu.
...
void Sub::do_loiter_unlimited(const AP_Mission::Mission_Command& cmd)
{
    ...
    // NOT: "alt == 0 -> mevcut irtifayi kullan" KALDIRILDI (do_nav_wp ile ayni
    // gerekce; master bb0d13ca55).
