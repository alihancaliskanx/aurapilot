 ...
 * -- The torpedo is below SURFTRAK_DEPTH
 *
...
bool ModeSurftrak::set_rangefinder_target_cm(float target_cm)
{
    ...
    if (torpedo.control_mode != Number::SURFTRAK) {
        torpedo.gcs().send_text(MAV_SEVERITY_WARNING, "wrong mode, rangefinder target not set");
    } else if (torpedo.inertial_nav.get_position_z_up_cm() >= torpedo.g.surftrak_depth) {
        torpedo.gcs().send_text(MAV_SEVERITY_WARNING, "descend below %f meters to set rangefinder target", torpedo.g.surftrak_depth * 0.01f);
    } else if (target_cm < (float)torpedo.rangefinder_state.min_cm) {
        torpedo.gcs().send_text(MAV_SEVERITY_WARNING, "rangefinder target below minimum, ignored");
    } else if (target_cm > (float)torpedo.rangefinder_state.max_cm) {
        torpedo.gcs().send_text(MAV_SEVERITY_WARNING, "rangefinder target above maximum, ignored");
    } else {
        success = true;
    }
    ...
    if (success) {
        ...
        torpedo.gcs().send_text(MAV_SEVERITY_INFO, "rangefinder target is %.2f meters", rangefinder_target_cm * 0.01f);

        // Initialize the terrain offset
        auto terrain_offset_cm = torpedo.inertial_nav.get_position_z_up_cm() - rangefinder_target_cm;
        torpedo.pos_control.set_pos_offset_z_cm(terrain_offset_cm);
        torpedo.pos_control.set_pos_offset_target_z_cm(terrain_offset_cm);
...
void ModeSurftrak::reset()
{
    ...
    torpedo.pos_control.set_pos_offset_z_cm(0);
    torpedo.pos_control.set_pos_offset_target_z_cm(0);
}
...
void ModeSurftrak::update_surface_offset()
{
    if (torpedo.rangefinder_alt_ok()) {
        ...
        float rangefinder_terrain_offset_cm = torpedo.rangefinder_state.rangefinder_terrain_offset_cm;

        // Handle the first reading or a reset
        if (!HAS_VALID_TARGET && torpedo.rangefinder_state.inertial_alt_cm < torpedo.g.surftrak_depth) {
            set_rangefinder_target_cm(torpedo.rangefinder_state.inertial_alt_cm - rangefinder_terrain_offset_cm);
        }

        if (HAS_VALID_TARGET) {
            // Will the new offset target cause the torpedo to ascend above SURFTRAK_DEPTH?
            float desired_z_cm = rangefinder_terrain_offset_cm + rangefinder_target_cm;
            if (desired_z_cm >= torpedo.g.surftrak_depth) {
                // Adjust the terrain offset to stay below SURFTRAK_DEPTH, this should avoid "at_surface" events
                rangefinder_terrain_offset_cm += torpedo.g.surftrak_depth - desired_z_cm;
            }

            // Set the offset target, AC_PosControl will do the rest
            torpedo.pos_control.set_pos_offset_target_z_cm(rangefinder_terrain_offset_cm);
        }
    }
}
