...
void ModeMotordetect::run()
{
    ...
    // Wait until torpedo stays for 500ms not spinning and leveled.
    case SETTLING:
        // Force all motors to stop
        for (uint8_t i=0; i <AP_MOTORS_MAX_NUM_MOTORS; i++) {
            if (motors.motor_is_enabled(i)) {
    ...
    case DONE:
        set_mode(torpedo.prev_control_mode, ModeReason::MISSION_END);
        torpedo.arming.disarm(AP_Arming::Method::MOTORDETECTDONE);
        break;
    }
}
