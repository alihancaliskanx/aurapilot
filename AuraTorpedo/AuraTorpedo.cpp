...
// AuraTorpedo scheduling, originally copied from ArduCopter

#include "Torpedo.h"

#define SCHED_TASK(func, rate_hz, max_time_micros, priority) SCHED_TASK_CLASS(Torpedo, &torpedo, func, rate_hz, max_time_micros, priority)
#define FAST_TASK(func) FAST_TASK_CLASS(Torpedo, &torpedo, func)
...
const AP_Scheduler::Task Torpedo::scheduler_tasks[] = {
    // update INS immediately to get current gyro data populated
    FAST_TASK_CLASS(AP_InertialSensor, &torpedo.ins, update),
    ...
    FAST_TASK_CLASS(AP_Mount, &torpedo.camera_mount, update_fast),
#endif

    SCHED_TASK(fifty_hz_loop,         50,     75,   3),
    SCHED_TASK_CLASS(AP_GPS, &torpedo.gps, update, 50, 200,   6),
#if AP_OPTICALFLOW_ENABLED
    SCHED_TASK_CLASS(AP_OpticalFlow,          &torpedo.optflow,             update,         200, 160,   9),
#endif
    ...
    SCHED_TASK_CLASS(AP_Baro,             &torpedo.barometer,    accumulate,          50,  90,  27),
    SCHED_TASK(one_hz_loop,            1,    100,  33),
    SCHED_TASK_CLASS(GCS,                 (GCS*)&torpedo._gcs,   update_receive,     400, 180,  36),
    SCHED_TASK_CLASS(GCS,                 (GCS*)&torpedo._gcs,   update_send,        400, 550,  39),
#if HAL_MOUNT_ENABLED
    SCHED_TASK_CLASS(AP_Mount,            &torpedo.camera_mount, update,              50,  75,  45),
#endif
#if AP_CAMERA_ENABLED
    SCHED_TASK_CLASS(AP_Camera,           &torpedo.camera,       update,              50,  75,  48),
#endif
#if HAL_LOGGING_ENABLED
    SCHED_TASK(ten_hz_logging_loop,   10,    350,  51),
    SCHED_TASK(twentyfive_hz_logging, 25,    110,  54),
    SCHED_TASK_CLASS(AP_Logger,           &torpedo.logger,       periodic_tasks,     400, 300,  57),
#endif
    SCHED_TASK_CLASS(AP_InertialSensor,   &torpedo.ins,          periodic,           400,  50,  60),
#if HAL_LOGGING_ENABLED
    SCHED_TASK_CLASS(AP_Scheduler,        &torpedo.scheduler,    update_logging,     0.1,  75,  63),
#endif
#if AP_RPM_ENABLED
    SCHED_TASK_CLASS(AP_RPM,              &torpedo.rpm_sensor,   update,              10, 200,  66),
#endif
    SCHED_TASK(terrain_update,        10,    100,  72),
#if AP_GRIPPER_ENABLED
    SCHED_TASK_CLASS(AP_Gripper,          &torpedo.g2.gripper,   update,              10,  75,  75),
#endif
...
void Torpedo::get_scheduler_tasks(const AP_Scheduler::Task *&tasks,
                                 uint8_t &task_count,
                                 uint32_t &log_bit)
{
...
constexpr int8_t Torpedo::_failsafe_priorities[5];

void Torpedo::run_rate_controller()
{
...
void Torpedo::update_batt_compass()
{
...
void Torpedo::ten_hz_logging_loop()
{
    ...
    if (should_log(MASK_LOG_NTUN) && (torpedo.flightmode->requires_GPS() || !torpedo.flightmode->has_manual_throttle())) {
...
void Torpedo::twentyfive_hz_logging()
{
...
bool Torpedo::control_check_barometer()
{
...
bool Torpedo::get_wp_distance_m(float &distance) const
{
    // see GCS_MAVLINK_Torpedo::send_nav_controller_output()
    distance = torpedo.wp_nav.get_wp_distance_to_destination() * 0.01;
    return true;
}

// vehicle specific waypoint info helpers
bool Torpedo::get_wp_bearing_deg(float &bearing) const
{
    // see GCS_MAVLINK_Torpedo::send_nav_controller_output()
    bearing = torpedo.wp_nav.get_wp_bearing_to_destination() * 0.01;
    return true;
}

// vehicle specific waypoint info helpers
bool Torpedo::get_wp_crosstrack_error_m(float &xtrack_error) const
{
    // no crosstrack error reported, see GCS_MAVLINK_Torpedo::send_nav_controller_output()
    xtrack_error = 0;
    return true;
}
...
/*
  update AP_Stats
*/
void Torpedo::stats_update(void)
{
...
bool Torpedo::ensure_ekf_origin()
{
    Location ekf_origin;
    ...
    auto backup_origin = Location(static_cast<int32_t>(torpedo.g2.backup_origin_lat * 1e7),
                                  static_cast<int32_t>(torpedo.g2.backup_origin_lon * 1e7),
                                  static_cast<int32_t>(torpedo.g2.backup_origin_alt * 100),
