...
bool ModeAuto::init(bool ignore_checks) {
     if (!torpedo.position_ok() || torpedo.mission.num_commands() < 2) {
        return false;
    }

    torpedo.auto_mode = Auto_Loiter;

    // stop ROI from carrying over from previous runs of the mission
    // To-Do: reset the yaw as part of auto_wp_start when the previous command was not a wp command to remove the need for this special ROI check
    if (torpedo.auto_yaw_mode == AUTO_YAW_ROI) {
        set_auto_yaw_mode(AUTO_YAW_HOLD);
    }

    // initialise waypoint controller
    torpedo.wp_nav.wp_and_spline_init();

    // clear guided limits
    guided_limit_clear();

    // start/resume the mission (based on MIS_RESTART parameter)
    torpedo.mission.start_or_resume();
    return true;
}
...
void ModeAuto::run()
{
    torpedo.mission.update();

    // call the correct auto controller
    switch (torpedo.auto_mode) {
...
void ModeAuto::auto_wp_run()
{
    ...
    if (!motors.armed()) {
        ...
        torpedo.wp_nav.wp_and_spline_init();                                                // Reset xy target
        return;
    }

    // process pilot's yaw input
    float target_yaw_rate = 0;
    if (!torpedo.failsafe.pilot_input) {
    ...
    // AURA torpido "pure pursuit" (Sub'in translate_wpnav_rp'si hover-varsayimli;
    // torpidoda dur-kalk kilitlenmesi yaratiyordu):
    //  - ileri: hedefe XY mesafesiyle orantili (15 m'den lineer inis) VE burun-hedef
    //    hizasiyla carpilir (hedef arkadaysa once don, sonra ilerle — kacan WP'ye doner)
    //  - yaw: hedefe-bearing (track-bearing DEGIL; WP gecilse bile geri dondurur)
    ...
    // Hiza carpani ALT SINIRLI (0.25): yanlis yone bakarken bile ilerleme kalir,
    // cunku donus otoritesi de finlerden = YOL SART (hizsiz donulemez, kilitlenir)
    const float trpd_hiza = MAX(0.25f, cosf(trpd_yaw_err_rad));
    float trpd_fwd = constrain_float(trpd_dist_cm / 1500.0f, 0.0f, 1.0f) * trpd_hiza;
    if (trpd_dist_cm > 1000.0f) {
        trpd_fwd = MAX(trpd_fwd, 0.35f);   // hedef 10 m'den uzakta: asla surunme hizina dusme
    }
    motors.set_lateral(0.0f);
    motors.set_forward(trpd_fwd);

    // AURA torpido: update_z_controller CAGRILMAZ (throttle yolu olu);
    // WP_Nav'in koydugu z hedefi pitch dongusuyle izlenir
    ...
    target_pitch = constrain_float(target_pitch + trpd_pitch_cd, -4000.0f, 4000.0f);
    ...
    if (torpedo.auto_yaw_mode == AUTO_YAW_HOLD) {
        ...
        // AURA torpido: yaw hedefi = hedefe-bearing (uzaktayken); hedefin dibinde
        // bearing gurultulenir -> get_auto_heading'e (aura yamali) birak
        const float trpd_yaw_cd = (trpd_dist_cm > 300.0f) ? trpd_wp_bearing_cd : get_auto_heading();
        attitude_control->input_euler_angle_roll_pitch_yaw(target_roll, target_pitch, trpd_yaw_cd, true);
    }
}

// auto_circle_movetoedge_start - initialise waypoint controller to move to edge of a circle with it's center at the specified location
//  we assume the caller has set the circle's circle with torpedo.circle_nav.set_center()
//  we assume the caller has performed all required GPS_ok checks
void ModeAuto::auto_circle_movetoedge_start(const Location &circle_center, float radius_m, bool ccw_turn)
{
    // set circle center
    torpedo.circle_nav.set_center(circle_center);

    // set circle radius
    if (!is_zero(radius_m)) {
        torpedo.circle_nav.set_radius_cm(radius_m * 100.0f);
    }

     // set circle direction by using rate
    float current_rate = torpedo.circle_nav.get_rate();
    current_rate = ccw_turn ? -fabsf(current_rate) : fabsf(current_rate);
    torpedo.circle_nav.set_rate(current_rate);

    // check our distance from edge of circle
    Vector3f circle_edge_neu;
    torpedo.circle_nav.get_closest_point_on_circle(circle_edge_neu);
    float dist_to_edge = (inertial_nav.get_position_neu_cm() - circle_edge_neu).length();

    // if more than 3m then fly to edge
    if (dist_to_edge > 300.0f) {
        // set the state to move to the edge of the circle
        torpedo.auto_mode = Auto_CircleMoveToEdge;
        ...
        if (!torpedo.wp_nav.set_wp_destination_loc(circle_edge)) {
            // failure to set destination can only be because of missing terrain data
            torpedo.failsafe_terrain_on_event();
        }

        // if we are outside the circle, point at the edge, otherwise hold yaw
        float dist_to_center = get_horizontal_distance_cm(inertial_nav.get_position_xy_cm().topostype(), torpedo.circle_nav.get_center().xy());
        if (dist_to_center > torpedo.circle_nav.get_radius() && dist_to_center > 500) {
            set_auto_yaw_mode(get_default_auto_yaw_mode(false));
        } else {
...
{
    torpedo.auto_mode = Auto_Circle;

    // initialise circle controller
    torpedo.circle_nav.init(torpedo.circle_nav.get_center(), torpedo.circle_nav.center_is_terrain_alt(), torpedo.circle_nav.get_rate());
}
...
{
    torpedo.mode_guided.init(true);
    torpedo.auto_mode = Auto_NavGuided;
    // initialise guided start time and position as reference for limit checking
    torpedo.mode_auto.guided_limit_init_time_and_pos();
}
...
bool ModeAuto::auto_loiter_start()
{
    ...
    torpedo.wp_nav.get_wp_stopping_point(stopping_point);

    // initialise waypoint controller target to stopping point
    torpedo.wp_nav.set_wp_destination(stopping_point);
...
void ModeAuto::auto_loiter_run()
{
    ...
    // AURA torpido: lateral yok; derinlik pitch dongusunde (throttle yolu olu)
    motors.set_lateral(0.0f);
    motors.set_forward(forward_out);
...
void ModeAuto::set_auto_yaw_look_at_heading(float angle_deg, float turn_rate_dps, int8_t direction, uint8_t relative_angle)
{
    ...
    if (relative_angle == 0) {
        // absolute angle
        torpedo.yaw_look_at_heading = wrap_360_cd(angle_deg * 100);
    } else {
        // relative angle
        if (direction < 0) {
            angle_deg = -angle_deg;
        }
        torpedo.yaw_look_at_heading = wrap_360_cd((angle_deg * 100 + curr_yaw_target));
    }

    // get turn speed
    if (is_zero(turn_rate_dps)) {
        // default to regular auto slew rate
        torpedo.yaw_look_at_heading_slew = AUTO_YAW_SLEW_RATE;
    } else {
        torpedo.yaw_look_at_heading_slew = MIN(turn_rate_dps, AUTO_YAW_SLEW_RATE);    // deg / sec
    }
...
{    
    // set torpedo to desired yaw rate
    torpedo.yaw_look_at_heading_slew = MIN(turn_rate_dps, AUTO_YAW_SLEW_RATE);    // deg / sec
...
void ModeAuto::set_auto_yaw_roi(const Location &roi_location)
{
    ...
    if (roi_location.alt == 0 && roi_location.lat == 0 && roi_location.lng == 0) {
        ...
        torpedo.camera_mount.clear_roi_target();
#endif  // HAL_MOUNT_ENABLED
    } else {
#if HAL_MOUNT_ENABLED
        // check if mount type requires us to rotate the torpedo
        if (!torpedo.camera_mount.has_pan_control()) {
            if (roi_location.get_vector_from_origin_NEU(torpedo.roi_WP)) {
                set_auto_yaw_mode(AUTO_YAW_ROI);
            }
        }
        // send the command to the camera mount
        torpedo.camera_mount.set_roi_target(roi_location);
...
#else
        // if we have no camera mount aim the torpedo at the location
        if (roi_location.get_vector_from_origin_NEU(torpedo.roi_WP)) {
            set_auto_yaw_mode(AUTO_YAW_ROI);
        }
#endif  // HAL_MOUNT_ENABLED
...
bool ModeAuto::auto_terrain_recover_start()
{
    ...
    switch (torpedo.rangefinder.status_orient(ROTATION_PITCH_270)) {
    ...
    case RangeFinder::Status::Good:
        torpedo.auto_mode = Auto_TerrainRecover;
        break;
    ...
    torpedo.fs_terrain_recover_start_ms = AP_HAL::millis();

    // Stop mission
    torpedo.mission.stop();

    // Reset xy target
    torpedo.loiter_nav.clear_pilot_desired_acceleration();
    torpedo.loiter_nav.init_target();

    // Reset z axis controller
    position_control->relax_z_controller(motors.get_throttle_hover());

    // initialize vertical maximum speeds and acceleration
    position_control->set_max_speed_accel_z(torpedo.wp_nav.get_default_speed_down(), torpedo.wp_nav.get_default_speed_up(), torpedo.wp_nav.get_accel_z());
    position_control->set_correction_speed_accel_z(torpedo.wp_nav.get_default_speed_down(), torpedo.wp_nav.get_default_speed_up(), torpedo.wp_nav.get_accel_z());
...
void ModeAuto::auto_terrain_recover_run()
{
    ...
    if (!motors.armed()) {
        ...
        torpedo.loiter_nav.init_target();                                                   // Reset xy target
    ...
    case RangeFinder::Status::OutOfRangeLow:
        target_climb_rate = torpedo.wp_nav.get_default_speed_up();
        rangefinder_recovery_ms = 0;
        break;

    case RangeFinder::Status::OutOfRangeHigh:
        target_climb_rate = torpedo.wp_nav.get_default_speed_down();
        rangefinder_recovery_ms = 0;
        break;
    ...
    case RangeFinder::Status::Good: // exit on success (recovered rangefinder data)
        ...
        if (torpedo.rangefinder_state.alt_healthy) {
            ...
            if (AP_HAL::millis() > rangefinder_recovery_ms + 1500) {
                ...
                torpedo.failsafe_terrain_set_status(true); // Reset failsafe timers
                torpedo.failsafe.terrain = false; // Clear flag
                torpedo.auto_mode = Auto_Loiter; // Switch back to loiter for next iteration
                torpedo.mission.resume(); // Resume mission
    ...
    default:
        ...
        torpedo.failsafe_terrain_act();
        rangefinder_recovery_ms = 0;
        return;
    }

    // exit on failure (timeout)
    if (AP_HAL::millis() > torpedo.fs_terrain_recover_start_ms + FS_TERRAIN_RECOVER_TIMEOUT_MS) {
    ...
    // AURA torpido: throttle yolu olu; derinlik pitch dongusunde
    const float trpd_pitch_cd = trpd_derinlik_pitch_cd();
