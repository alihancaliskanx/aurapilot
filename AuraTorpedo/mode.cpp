...
Mode::Mode(void) :
    g(torpedo.g),
    g2(torpedo.g2),
    inertial_nav(torpedo.inertial_nav),
    ahrs(torpedo.ahrs),
    motors(torpedo.motors),
    channel_roll(torpedo.channel_roll),
    channel_pitch(torpedo.channel_pitch),
    channel_throttle(torpedo.channel_throttle),
    channel_yaw(torpedo.channel_yaw),
    channel_forward(torpedo.channel_forward),
    channel_lateral(torpedo.channel_lateral),
    position_control(&torpedo.pos_control),
    attitude_control(&torpedo.attitude_control),
    G_Dt(torpedo.G_Dt)
{ };

// return the static controller object corresponding to supplied mode
Mode *Torpedo::mode_from_mode_num(const Mode::Number mode)
{
    Mode *ret = nullptr;
...
bool Torpedo::set_mode(Mode::Number mode, ModeReason reason)
{
    ...
    if (!torpedo.control_check_barometer() && // maybe use ekf_alt_ok() instead?
...
void Torpedo::exit_mode(Mode::Number old_control_mode, Mode::Number new_control_mode)
{
...
bool Torpedo::set_mode(const uint8_t new_mode, const ModeReason reason)
{
    static_assert(sizeof(Mode::Number) == sizeof(new_mode), "The new mode can't be mapped to the vehicles mode number");
    return torpedo.set_mode(static_cast<Mode::Number>(new_mode), reason);
}

// update_flight_mode - calls the appropriate attitude controllers based on flight mode
// called at 100hz or more
void Torpedo::update_flight_mode()
{
    flightmode->run();
}

// exit_mode - high level call to organise cleanup as a flight mode is exited
void Torpedo::exit_mode(Mode *&old_flightmode, Mode *&new_flightmode){
#if HAL_MOUNT_ENABLED
        camera_mount.set_mode_to_default();
#endif  // HAL_MOUNT_ENABLED
}
...
void Torpedo::notify_flight_mode()
{
...
void Mode::get_pilot_desired_angle_rates(int16_t roll_in, int16_t pitch_in, int16_t yaw_in, float &roll_out, float &pitch_out, float &yaw_out)
{
    ...
    if (g.acro_trainer != ACRO_TRAINER_DISABLED) {
        ...
        if (g.acro_trainer == ACRO_TRAINER_LIMITED) {
            if (roll_angle > torpedo.aparm.angle_max) {
                rate_ef_level.x -=  g.acro_balance_roll*(roll_angle-torpedo.aparm.angle_max);
            } else if (roll_angle < -torpedo.aparm.angle_max) {
                rate_ef_level.x -=  g.acro_balance_roll*(roll_angle+torpedo.aparm.angle_max);
            }

            if (pitch_angle > torpedo.aparm.angle_max) {
                rate_ef_level.y -=  g.acro_balance_pitch*(pitch_angle-torpedo.aparm.angle_max);
            } else if (pitch_angle < -torpedo.aparm.angle_max) {
                rate_ef_level.y -=  g.acro_balance_pitch*(pitch_angle+torpedo.aparm.angle_max);
            }
        }
...
{
    return torpedo.set_mode(mode, reason);
}

GCS_Torpedo &Mode::gcs()
{
    return torpedo.gcs();
}

// ===== AURA torpido: derinlik -> pitch dis dongusu (Faz 3.2/3.3; ortak yardimci) =====
// Kazanclar ilk surum sabitleri; parametrelestirme Faz 2.4/3 kalani.
static constexpr float TRPD_Z2P_KP_CD   = 1200.0f;  // cd/m   (1 m hata -> 12 der pitch)
static constexpr float TRPD_Z2P_KD_CD   = 900.0f;   // cd/(m/s) dikey hiz sonumu
static constexpr float TRPD_Z2P_MAX_CD  = 3000.0f;  // pitch talebi siniri (±30 der)
static constexpr float TRPD_Z2P_MIN_SPD = 0.8f;     // m/s; altinda fin otoritesi yok

float Mode::trpd_derinlik_pitch_cd() const
{
    const float hedef_z_cm = position_control->get_pos_target_z_cm();
