#include "AP_Arming_Torpedo.h"
#include "Torpedo.h"

bool AP_Arming_Torpedo::rc_calibration_checks(bool display_failure)
{
    const RC_Channel *channels[] = {
        torpedo.channel_roll,
        torpedo.channel_pitch,
        torpedo.channel_throttle,
        torpedo.channel_yaw
    };
    return rc_checks_copter_sub(display_failure, channels);
}

bool AP_Arming_Torpedo::has_disarm_function() const {
    ...
    for (uint8_t i = 0; i < 16; i++) {
        switch (torpedo.get_button(i)->function(false)) {
    ...
    if (has_shift_function) {
        for (uint8_t i = 0; i < 16; i++) {
            switch (torpedo.get_button(i)->function(true)) {
                case JSButton::k_arm_toggle :
                case JSButton::k_disarm :
                    return true;
            }
...
bool AP_Arming_Torpedo::pre_arm_checks(bool display_failure)
{
    if (armed) {
        return true;
    }
...
bool AP_Arming_Torpedo::ins_checks(bool display_failure)
{
    ...
    // additional torpedo-specific checks
...
bool AP_Arming_Torpedo::arm(AP_Arming::Method method, bool do_arming_checks)
{
    ...
    torpedo.mainloop_failsafe_disable();
    ...
    torpedo.initial_armed_bearing = ahrs.yaw_sensor;
    ...
    if (!ahrs.home_is_set()) {
        ...
        if (!torpedo.set_home_to_current_location(false)) {
            // ignore this failure
        }
    }
    ...
    torpedo.enable_motor_output();

    // finally actually arm the motors
    torpedo.motors.armed(true);

#if HAL_LOGGING_ENABLED
    // log flight mode in case it was changed while vehicle was disarmed
    AP::logger().Write_Mode((uint8_t)torpedo.control_mode, torpedo.control_mode_reason);
#endif

    // reenable failsafe
    torpedo.mainloop_failsafe_enable();
...
bool AP_Arming_Torpedo::disarm(const AP_Arming::Method method, bool do_disarm_checks)
{
    ...
    torpedo.motors.armed(false);

    // reset the mission
    torpedo.mission.reset();
